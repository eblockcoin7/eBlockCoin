...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EBCR);
    ...
    if (!settings.contains("nAnonymizeeBlockCoinAmount"))
        settings.setValue("nAnonymizeeBlockCoinAmount", 1000);

    nAnonymizeeBlockCoinAmount = settings.value("nAnonymizeeBlockCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeeBlockCoinAmount"))
        SoftSetArg("-anonymizeeblockcoinamount", settings.value("nAnonymizeeBlockCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eblockcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeeBlockCoinAmount:
            return QVariant(nAnonymizeeBlockCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeeBlockCoinAmount:
            nAnonymizeeBlockCoinAmount = value.toInt();
            settings.setValue("nAnonymizeeBlockCoinAmount", nAnonymizeeBlockCoinAmount);
            emit anonymizeeBlockCoinAmountChanged(nAnonymizeeBlockCoinAmount);
            break;
        case CoinControlFeatures:
