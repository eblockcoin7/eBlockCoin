...
#include "ui_zebcrcontroldialog.h"
...
std::set<std::string> ZEbcrControlDialog::setSelectedMints;
std::set<CMintMeta> ZEbcrControlDialog::setMints;

ZEbcrControlDialog::ZEbcrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEbcrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZEbcrControlDialog::~ZEbcrControlDialog()
{
    delete ui;
}

void ZEbcrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEbcrControlDialog::updateList()
{
...
void ZEbcrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEbcrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEbcr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEbcrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEbcrControlDialog::GetSelectedMints()
{
...
void ZEbcrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
