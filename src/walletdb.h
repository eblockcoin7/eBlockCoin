...
public:
    ...
    bool WriteZEBCRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZEBCRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZEBCRSeed_deprecated(uint256& seed);
    bool EraseZEBCRSeed();
    bool EraseZEBCRSeed_deprecated();

    bool WriteZEBCRCount(const uint32_t& nCount);
    bool ReadZEBCRCount(uint32_t& nCount);
