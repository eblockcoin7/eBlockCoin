...
#ifndef eBlockCoin_ZEBCRWALLET_H
#define eBlockCoin_ZEBCRWALLET_H
...
public:
    CzEBCRWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZEBCR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZEBCR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //eBlockCoin_ZEBCRWALLET_H
