...
#if defined(NDEBUG)
#error "eBlockCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uebcr) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zEBCR spend with serial %s is already in block %d\n", __func__,
...
{
	//Check to see if the zEBCR is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zEBCR spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zEBCR without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zEBCR spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zEBCR spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zebcr");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zEBCR mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 240000 && nHeight > 200000) {
		nSubsidy = 20 * COIN;
	}
	...
	else if (nHeight <= 280020 && nHeight > 280000) {
		nSubsidy = 20 * COIN;
	}
	else if (nHeight <= 300000 && nHeight > 280020) {
		nSubsidy = 24 * COIN;
	}
	else if (nHeight <= 330000 && nHeight > 300000) {
		nSubsidy = 26 * COIN;
	}
	else if (nHeight <= 360000 && nHeight > 330000) {
		nSubsidy = 28 * COIN;
	}
	else if (nHeight <= 400000 && nHeight > 360000) {
		nSubsidy = 25 * COIN;
	}
	...
	else if (nHeight <= 4173038 && nHeight > 1000000) {
		nSubsidy = 10 * COIN;
	}		
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZEBCRStake)
{
	...
	else if (nHeight <= 200000 && nHeight > Params().LAST_POW_BLOCK()) {
		ret = blockValue * 4 / 5;  //80%;
	}
	else if (nHeight <= 450000 && nHeight > 200000) {
		ret = blockValue * 17 / 20;  //85%;
	}
	else if (nHeight <= 750000 && nHeight > 450000) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from eBlockCoin
...
{
	RenameThread("eblockcoin-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZEBCRMinted()
{
...
void RecalculateZEBCRSpent()
{
	...
	while (true) {
		...
		//Remove spends from zEBCR supply
...
bool RecalculateEBCRSupply(int nHeightStart)
{
...
bool UpdateZEBCRSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		...
		RecalculateEBCRSupply(Params().Zerocoin_StartHeight());
	}

	//Track zEBCR money supply in the block index
	if (!UpdateZEBCRSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zEBCR supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zEbcrSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	// If turned on AutoZeromint will automatically convert EBCR to zEBCR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zEBCR spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zEBCR serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zEBCR stake block is less than allowed start height", __func__);

	if (CZEbcrStake* zEBCR = dynamic_cast<CZEbcrStake*>(stake)) {
		CBlockIndex* pindexFrom = zEBCR->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zEBCR stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zEBCR stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zEBCR->GetValue()));
		if (nChecksum200 != zEBCR->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zEBCR->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZEBCR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zEBCR fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> ebcrInputs;
		std::vector<CTxIn> zEBCRInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zEBCRInputs.push_back(stakeIn);
			}
			else {
				ebcrInputs.push_back(stakeIn);
			}
		}
		const bool hasEBCRInputs = !ebcrInputs.empty();
		const bool hasZEBCRInputs = !zEBCRInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasEBCRInputs)
					...
					for (const CTxIn& ebcrIn : ebcrInputs) {
						if (ebcrIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			do {
				...
				for (const CTransaction& t : bl.vtx) {
					for (const CTxIn& in : t.vin) {
						...
						for (const CTxIn& stakeIn : ebcrInputs) {
			...
			// Now that this loop if completed. Check if we have zEBCR inputs.
			if (hasZEBCRInputs) {
				for (const CTxIn& zEbcrInput : zEBCRInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zEbcrInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zebcr");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zEBCR mints and %d zEBCR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// eBlockCoin: We use certain sporks during IBD, so check to see if they are
