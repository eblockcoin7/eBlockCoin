...
CzEBCRWallet::CzEBCRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zebcr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEBCRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEBCRSeed_deprecated()) {
                    LogPrintf("%s: Updated zEBCR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zebcr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zebcr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEBCRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEBCRCount(nCountLastUsed);
    else if (!walletdb.ReadZEBCRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEBCRWallet::Lock()
{
    seedMaster = 0;
}

void CzEBCRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEBCRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEBCR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEBCRWallet::LoadMintPoolFromDB()
{
...
void CzEBCRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEBCRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEBCRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zebcrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEBCRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEBCR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zebcrTracker which also adds to database
    pwalletMain->zebcrTracker->Add(dMint, true);
...
void CzEBCRWallet::SeedToZEBCR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEBCRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEBCRWallet::GenerateDeterministicZEBCR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEBCRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEBCRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
