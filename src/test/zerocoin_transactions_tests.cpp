...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zebcrTracker = unique_ptr<CzEBCRTracker>(new CzEBCRTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZEBCR_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZEBCR_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
